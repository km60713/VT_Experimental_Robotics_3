...
def urlExecution(command, param, value):
	timeStamp = int(time.time())*1000
        if(command != -1):
                ip = 'http://192.168.1.37:81/decoder_control.cgi?loginuse=admin&loginpas=12345&command='
                oneStep = '&onestep=0&'
                gibberish = '7485621407675288&_='
                fullURL = ip+str(command)+oneStep+str(timeStamp)+'.49641236611690986&_='+str(timeStamp)
        else:
                ip = 'http://192.168.1.37:81/camera_control.cgi?loginuse=admin&loginpas=12345&param='
                gibberish = '7485621407675288&_='
...
def callback(data):
	# rospy.loginfo(data)
	# rospy.loginfo('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
	horizontal = data.axes[4] # 14, 15 (14: left)
	vertical = data.axes[5] # 8 10 (8: upper)
...
def listener():
    ...
    rospy.Subscriber("/cmd_key", Joy, callback)
